...
func SkuFromRow(row *sql.Row) (*SKU, error) {
	var skuRes SKU
	err := row.Scan(&skuRes.Sku, &skuRes.Name, &skuRes.Description, &skuRes.Barcode)
...
func CreateSku(accountID int64, sku string, name string, description string, barcode string) (*SKU, error) {
	row := database.GetDB().QueryRow("INSERT INTO wms.skus (account_id, sku, name, description, barcode) VALUES($1, $2, $3, $4, $5) RETURNING sku, name, description, barcode", accountID, sku, name, description, barcode)
	return SkuFromRow(row)
}

// FindSku will lokkup a sku by name
func FindSku(sku string) (*SKU, error) {
	row := database.GetDB().QueryRow("SELECT sku, name, description, barcode FROM wms.skus WHERE sku = '$1'", sku)
	return SkuFromRow(row)
}
